...
export function FontSettings(props) {
  ...
  return <>
			<ThemeProvider theme={darkTheme}>
				<Autocomplete multiple value={fontFamily} onChange={(event, newValue) => {
        setFontFamily(newValue);
      }} options={fontList} getOptionLabel={option => option} defaultValue={[]} fullWidth freeSolo forcePopupIcon={false} renderInput={params => <TextField {...params} variant="outlined" label="选择字体" placeholder="" />} />
			</ThemeProvider>
...
const FontPreset = memo(function FontPreset(props) {
  const hasFont = props.fonts.some(font => props.fontList.includes(font));
  return <div className="rnp-font-preset">
			<label className="rnp-font-preset-label">{props.name}</label>
			{hasFont && <button className="rnp-font-preset-button" onClick={() => props.setFontFamily(props.fonts)}>应用</button>}
			{!hasFont && <button className="rnp-download-font-button" onClick={async () => {
