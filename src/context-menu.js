...
function ContextMenu(props) {
  ...
  return <div className="rnp-context-menu" tabIndex={0} ref={menuRef}>
			{props.items.map((item, index) => item.divider ? <div className="rnp-context-menu-devider" key={index} /> : <div key={index} className="rnp-context-menu-item" onClick={() => {
      if (item.callback) {
        item.callback();
      }
      closeMenu();
    }}>
					{item.html ? <div dangerouslySetInnerHTML={{
        __html: item.html
      }} /> : item.label}
				</div>)}
		</div>;
