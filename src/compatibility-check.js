...
function Wizard(props) {
  ...
  useEffect(async () => {
    try {
      if (typeof betterncm_native == "undefined" || typeof betterncm.app.writeConfig == "undefined" || typeof betterncm.app.readConfig == "undefined" || typeof betterncm_native.app.reloadIgnoreCache == "undefined") {
  ...
  useEffect(async () => {
    ...
    try {
      if ((await betterncm.app.readConfig("cc.microblock.betterncm.remove-disable-gpu")) != "true" && (await new Promise((resolve, reject) => {
        channel.call("app.getLocalConfig", GpuAccelerationEnabled => {
  ...
  useEffect(async () => {
    if (typeof betterncm.app.readConfig == "undefined") return;
    try {
      if ((await betterncm.app.readConfig("cc.microblock.betterncm.cpp_side_inject_feature_disabled")) == "true") setIsHijackDisabled(true);
    } catch (e) {}
  }, []);
  useEffect(() => {
  ...
  return <div class="rnp-compatibility-check">
			...
			<div class="rnp-compatibility-check__content">
				...
				{isNCMOutdated && <>
						...
						<Button text="下载新版网易云" disabledAfterDone={false} onClick={useCallback(async () => {
          await betterncm.app.exec("https://music.163.com/#/download");
				...
				<Button text="下载 BetterNCM Installer" disabledAfterDone={false} onClick={useCallback(async () => {
        await betterncm.app.exec("https://github.com/MicroCBer/BetterNCM-Installer/releases");
				...
				{isGPUDisabled ? <p className="warning">检测到您的 GPU 加速已被禁用，可能会导致 Refined Now Playing 无法正常工作。请启用 GPU 加速。</p> : <p className="pass">未检测到您的 GPU 加速被禁用。但如果仍旧出现以上问题，请尝试使用以下的按钮启用 GPU 加速。</p>}
				<Button text="启用 GPU 加速" disabledAfterDone={true} onClick={useCallback(async () => {
        ...
        await betterncm_native.app.restart();
				...
				{isHijackDisabled ? <p className="warning">检测到您的 Hijack JS 注入已被禁用。请启用 Hijack JS 注入。</p> : <p className="pass">Hijack JS 注入未被禁用。但如果仍旧无法显示歌词，请点击以下 "清空 Hijack 缓存按钮"。</p>}
				{<Button text="启用 Hijack JS 注入" disabledAfterDone={true} disabled={!isHijackDisabled} onClick={useCallback(async () => {
      ...
      }, [setIsHijackDisabled])} clickedText="已启用 Hijack JS 注入" />}
				{!isHijackDisabled && <Button text="清空 Hijack 缓存" disabledAfterDone={true} onClick={useCallback(async () => {
        betterncm_native.app.reloadIgnoreCache();
      }, [])} />}
				<h1>性能</h1>
				{<>
        ...
        betterncm_native.app.reloadIgnoreCache();
				...
				{(isNCMOutdated || isBetterNCMOutdated || isGPUDisabled || isHijackDisabled) && <>
						<Button text="跳过" disabledAfterDone={true} onClick={useCallback(() => {
          document.querySelector("#refined-now-playing-wizard").remove();
        }, [])} />
						<Button text="跳过并不再提示" disabledAfterDone={true} onClick={useCallback(() => {
          localStorage.setItem("refined-now-playing-wizard-done", "true");
          document.querySelector("#refined-now-playing-wizard").remove();
        }, [])} />
					</>}
...
const Button = memo(function Button(props) {
  ...
  return <button class="action-button" disabled={disabled || props.disabled} onClick={async () => {
    if (disabled) return;
    setDisabled(true);
    props.onClick();
    setClicked(true);
...
const HijackFailureNotice = memo(function HijackFailureNotice() {
  const clicked = useRef(false);
  if (clicked.current) {
    return null;
  }
  return <div className="hijack-failure-notice">
			<div className='info'>
				<div>Hijack 错误</div>
