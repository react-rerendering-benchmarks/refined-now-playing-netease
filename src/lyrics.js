...
const useRefState = initialValue => {
  ...
  return [value.current, setValue, ref];
};
export function Lyrics(props) {
  ...
  const recalcCounter = useRef(0); // 手动重计算时触发渲染
  ...
  const currentLineForScrolling = useRef(0); // 为提前 0.2s 滚动，使滚动 delay 与逐词歌词对应 而设置的 提前的，仅用于滚动的 currentLine
  ...
  const lyricFade = useRef(getSetting('lyric-fade', false));
  const lyricZoom = useRef(getSetting('lyric-zoom', false));
  const lyricBlur = useRef(getSetting('lyric-blur', false));
  const lyricRotate = useRef(getSetting('lyric-rotate', false));
  const RotateCurvature = useRef(getSetting('lyric-rotate-curvature', 30));
  ...
  const currentLyricAlignmentPercentage = useRef(parseInt(getSetting('current-lyric-alignment-percentage', 50)));
  const lyricStagger = useRef(getSetting('lyric-stagger', true));
  const [lyricGlow, setLyricGlow] = useState(getSetting('lyric-glow', true));
  const [overviewMode, setOverviewMode] = useState(false);
  const overviewModeScrolling = useRef(false);
  ...
  setScrollingMode = x => {
    ...
    if (x) containerRef.current.classList.add('scrolling');else containerRef.current.classList.remove('scrolling');
    _setScrollingMode(x);
  };
  const isPureMusic = lyrics && (lyrics.length === 1 || lyrics.length <= 10 && lyrics.some(x => (x.originalLyric ?? '').includes('纯音乐')));
  ...
  const onLyricsUpdate = e => {
    ...
    if (!e.detail.amend) {
      ...
      setCurrentLine(0);
      currentLineForScrolling.current = 0;
    }
    ...
    if (e.detail.amend) {
      ...
      recalcCounter.current = +new Date();
    }
  };
  useEffect(() => {
    shouldTransit.current = false;
  ...
  }, [lyrics, containerWidth.current, fontSize, showTranslation, showRomaji, useKaraokeLyrics, karaokeAnimation, recalcCounter.current]);
  ...
  useEffect(() => {
    ...
    const delayByOffset = offset => {
      ...
      if (currentLineForScrolling.current == previousFocusedLineRef.current || scrollingMode) {
        return 0;
      }
      if (!lyricStagger.current) {
        return 0;
      }
      let sign = currentLineForScrolling.current - previousFocusedLineRef.current > 0 ? 1 : -1;
      offset = Math.max(-4, Math.min(4, offset)) * sign + 4;
      return offset * 50;
    };
    const scaleByOffset = offset => {
      if (!lyricZoom.current) return 1;
      if (customScaleFunc) {
        try {
    ...
    const blurByOffset = offset => {
      if (!lyricBlur.current || scrollingMode) return 0;
      if (customBlurFunc) {
        try {
    ...
    const opacityByOffset = offset => {
      if (!lyricFade.current || scrollingMode) return 1;
    ...
    const setRotateTransform = (line, yOffset, height) => {
      if (!lyricRotate.current) return;
      const origin = [-120 + (RotateCurvature.current - 25), -(yOffset + height / 2)];
      const len = Math.sqrt(origin[0] * origin[0] + origin[1] * origin[1]);
      line.rotate = Math.min(yOffset / window.innerHeight * -RotateCurvature.current, 90);
      ...
      if (opacity <= -1.5) line.outOfRangeHidden = true;else if (line.outOfRangeHidden) delete line.outOfRangeHidden;
    };

    //console.log(currentLine, previousFocusedLineRef.current, currentLine - previousFocusedLineRef.current > 0 ? 1 : -1);

    const transforms = [];
    for (let i = 0; i <= lyrics.length; i++) transforms.push({
      top: 0,
      scale: 1,
      delay: 0
    });
    //console.log('containerHeight', containerHeight);
    let current = Math.min(Math.max(currentLineForScrolling.current ?? 0, 0), lyrics.length - 1);
    ...
    transforms[current].top = containerRef.current.clientHeight * (currentLyricAlignmentPercentage.current * 0.01) - heightOfItems.current[current] / 2;
    ...
    previousFocusedLineRef.current = currentLineForScrolling.current;
  }, [currentLineForScrolling.current, containerHeight, containerWidth.current, fontSize, lyricFade.current, lyricZoom.current, lyricBlur.current, lyricRotate.current, RotateCurvature.current, showTranslation, showRomaji, useKaraokeLyrics, scrollingMode, scrollingFocusLine, currentLyricAlignmentPercentage.current, lyricStagger.current, recalcCounter.current, lyrics]);
  const onPlayStateChange = (id, state) => {
    ...
    if (document.querySelector(".m-player-fm .btnp").classList.contains("btnp-pause")) {
      currentLineForScrolling.current = currentLine;
    }
    setSongId(id);
  };
  const onPlayProgress = (id, progress) => {
    ...
    if (cur == _lyrics.current.length - 1 && _lyrics.current[cur].duration && currentTimeWithOffset > _lyrics.current[cur].time + _lyrics.current[cur].duration + 500) {
      cur = _lyrics.current.length;
    }
    let curForScrolling = Math.max(0, cur - 1);
    const scrollingDelay = lyricStagger.current ? 200 : 0;
    for (let i = startIndex; i < _lyrics.current.length; i++) {
    ...
    setCurrentLine(cur);
    currentLineForScrolling.current = curForScrolling;
  };
  useEffect(() => {
  ...
  const jumpToTime = useCallback(time => {
    ...
    setScrollingMode(false);
    //console.log(songId);
    channel.call("audioplayer.seek", () => {}, [songId, `${songId}|seek|${Math.random().toString(36).substring(6)}`, time / 1000]);
    ...
    setSeekCounter(+new Date());
    if (!playState) {
      if (!isFM) document.querySelector("#main-player .btnp").click();else document.querySelector(".m-player-fm .btnp").click();
    }
  ...
  useEffect(() => {
    ...
    const onLyricFadeChange = e => {
      lyricFade.current = e.detail ?? false;
    };
    const onLyricZoomChange = e => {
      lyricZoom.current = e.detail ?? false;
    };
    const onLyricBlurChange = e => {
      lyricBlur.current = e.detail ?? false;
    };
    const onLyricRotateChange = e => {
      lyricRotate.current = e.detail ?? false;
    };
    const onRotateCurvatureChange = e => {
      RotateCurvature.current = e.detail ?? 30;
    };
    ...
    const onCurrentLyricAlignmentPercentageChange = e => {
      currentLyricAlignmentPercentage.current = parseInt(e.detail) ?? 50;
    };
    const onLyricStaggerChange = e => {
      lyricStagger.current = e.detail ?? true;
    };
  ...
  useLayoutEffect(() => {
    ...
    if (overviewModeScrolling.current) return;
  ...
  }, [currentLine, overviewModeScrolling.current]);
  const exitOverviewModeScrollingSoon = useCallback((timeout = 3000) => {
    ...
    exitOverviewModeScrollingTimeout.current = setTimeout(() => {
      overviewModeScrolling.current = false;
    }, timeout);
  }, []);
  ...
  return <>
			<div className={`rnp-lyrics ${isPureMusic ? 'pure-music' : ''} ${overviewMode || isUnsynced ? 'overview-mode-hide' : ''}`} ref={containerRef} style={{
      fontSize: `${fontSize}px`
    }}>
				{lyrics && lyrics.map((line, index) => {
        return <Line key={`${songId} ${index}`} id={index} line={line} currentLine={currentLine} currentTime={currentTime.current + globalOffset} seekCounter={seekCounter} playState={playState} showTranslation={showTranslation} showRomaji={showRomaji} useKaraokeLyrics={useKaraokeLyrics} jumpToTime={isPureMusic ? () => {} : jumpToTime} transforms={lineTransforms[index] ?? {
          top: 0,
          scale: 1,
          delay: 0,
          blur: 0
        }} karaokeAnimation={karaokeAnimation} outOfRangeScrolling={scrollingMode && length > 100 && Math.abs(index - scrollingFocusLine) > 20} outOfRangeKaraoke={/*length > 100 && */Math.abs(index - currentLine) > 10} lyricGlow={lyricGlow} />;
      })}
				<Contributors transforms={lineTransforms[lineTransforms.length - 1] ?? {
        top: 0,
        scale: 1,
        delay: 0,
        blur: 0
			...
			<Scrollbar nonInterludeToAll={nonInterludeToAllLyricsMapping} allToNonInterlude={allToNonInterludeLyricsMapping} currentLine={currentLine} containerHeight={containerHeight} scrollingMode={scrollingMode} scrollingFocusLine={scrollingFocusLine} scrollingFocusOnLine={scrollingFocusOnLine} exitScrollingModeSoon={exitScrollingModeSoon} overviewMode={overviewMode || isUnsynced} />
        ...
        }} xmlns="http://www.w3.org/2000/svg" height="20" width="20"><path d="M4.146 14.854v-1.396h6.792v1.396Zm0-2.937v-1.396h11.729v1.396Zm0-2.959V7.562h11.729v1.396Zm0-2.937V4.625h11.729v1.396Z" /></svg>
					</button>}
				{(overviewMode || isUnsynced) && <button className={`
							rnp-lyrics-switch-btn
							rnp-lyrics-switch-btn-top
        ...
        display: 'none',
        marginBottom: '10px'
      }} title="使用 ChatGPT 生成翻译" onClick={async () => {
        ...
        setShowRomaji(!showRomaji);
				...
				<button className={`rnp-lyrics-switch-btn
        ...
        setUseKaraokeLyrics(!useKaraokeLyrics);
			...
			{(overviewMode || isUnsynced) && <LyricOverview lyrics={lyrics} currentLine={currentLine} showRomaji={showRomaji} showTranslation={showTranslation} jumpToTime={jumpToTime} isUnsynced={isUnsynced} overviewContainerRef={overviewContainerRef} setOverviewModeScrolling={setOverviewModeScrolling} exitOverviewModeScrollingSoon={exitOverviewModeScrollingSoon} />}
		</>;
}
function Line(props) {
  if (props.outOfRangeScrolling) {
    return <div className={`rnp-lyrics-line ${props.line.isInterlude ? 'rnp-interlude' : ''}`} offset={offset} style={{
      display: 'none'
    }} />;
  }
  ...
  return <div className={`rnp-lyrics-line ${offset < 0 ? 'passed' : ''} ${props.line.isInterlude ? 'rnp-interlude' : ''}`} offset={offset} onClick={() => props.jumpToTime(props.line.time + 50)} onContextMenu={e => {
    e.preventDefault();
    ...
    ...(props.transforms?.outOfRangeHidden && {
      visibility: 'hidden'
    })
  }}>
        ...
        return <div key={`${props.karaokeAnimation} ${index}`} ref={karaokeLineRef.current?.children[index]} className={`rnp-karaoke-word ${word?.isCJK ? 'is-cjk' : ''} ${word?.endsWithSpace ? 'end-with-space' : ''}`} style={getKaraokeAnimation(word)}>
							<span>{word.word}</span>
			...
			{props.line.isInterlude && <Interlude id={props.id} line={props.line} currentLine={props.currentLine} currentTime={props.currentTime} seekCounter={props.seekCounter} playState={props.playState} />}
		</div>;
}
function Interlude(props) {
  ...
  return <div className="rnp-interlude-inner" ref={dotContainerRef}>
			{dots.map((dot, index) => {
...
function Contributors(props) {
  ...
  return <div className="rnp-contributors" style={{
    transform: `
					${props.transforms.left ? `translateX(${props.transforms.left}px)` : ''}
					translateY(${props.transforms.top + (props.transforms?.extraTop ?? 0)}px)
					scale(${props.transforms.scale})
...
function Artist(props) {
  ...
  return <div className="rnp-contributor rnp-contributor-artist">
			<span>{props.role.roleName}: </span>
			{props.role.artistMetaList.map((artist, index) => {
      return <>
							{artist.artistId ? <a className="rnp-contributor-artist" href={`#/m/artist/?id=${artist.artistId}`}>
									{artist.artistName}
								</a> : <span className="rnp-contributor-artist">
									{artist.artistName}
								</span>}
...
function Contributor(props) {
  ...
  return <div className="rnp-contributor rnp-contributor-lyrics">
			<span>{props.text}: </span>
			{props?.user?.userid ? <a className="rnp-contributor-user" href={`#/m/personal/?uid=${props.user.userid}`}>
					{props.user.name}
				</a> : <span className="rnp-contributor-user">
					{props.user.name}
				</span>}
		</div>;
}
function Scrollbar(props) {
  ...
  return <div className={`rnp-lyrics-scrollbar ${props.overviewMode ? 'overview-mode-hide' : ''}`} ref={scrollbarRef}>
			<div className={`rnp-lyrics-scrollbar-thumb ${totalSteps > 1 ? '' : 'no-scroll'}`} ref={thumbRef} style={{
      height: thumbHeight,
...
const LyricOverview = memo(function LyricOverview(props) {
  useEffect(() => {
  ...
  return <div className="rnp-lyrics rnp-lyrics-overview-container" ref={props.overviewContainerRef}>
        ...
        return !(props.isUnsynced && index == 0) && <div key={index} className={`
