...
const addSettingsMenu = async (isFM = false) => {
  ...
  const bindSliderToCSSVariable = (slider, variable, defaultValue = 0, event = 'input', mapping = x => {
    return x;
  ...
  const bindSliderToFunction = (slider, func, defaultValue = 0, event = 'input', mapping = x => {
    return x;
  }, addClassWhenAdjusting = '') => {
    slider.value = getSetting(slider.id, defaultValue);
    slider.dispatchEvent(new Event("input"));
  ...
  const bindSelectGroupToClasses = (selectGroup, defaultValue, mapping = x => {
    return x;
  }, callback = x => {}) => {
  ...
  const initSettings = () => {
    ...
    bindSelectGroupToClasses(accentColorVariant, 'primary', x => `accent-color-${x}`, x => {
      if (x == 'off') document.body.classList.remove('enable-accent-color');else document.body.classList.add('enable-accent-color');
    ...
    bindSelectGroupToClasses(horizontalAlign, 'left', x => {
      return `horizontal-align-${x}`;
    }, () => {
      recalculateTitleSize();
    });
    bindSelectGroupToClasses(verticalAlign, 'bottom', x => {
    ...
    bindCheckboxToClass(coverBlurryShadow, 'cover-blurry-shadow', true, x => {
      document.dispatchEvent(new CustomEvent('rnp-cover-shadow-type', {
        detail: {
          type: x ? 'colorful' : 'black'
        }
      }));
    });
    ...
    bindSelectGroupToClasses(backgroundType, 'blur', x => `rnp-bg-${x}`, x => {
      document.dispatchEvent(new CustomEvent('rnp-background-type', {
        detail: {
          type: x
        }
      }));
    ...
    bindCheckboxToClass(staticFluid, 'static-fluid', false, x => {
      document.dispatchEvent(new CustomEvent('rnp-static-fluid', {
        detail: x
      }));
    });
    ...
    bindSliderToFunction(lyricFontSize, x => {
      document.dispatchEvent(new CustomEvent('rnp-lyric-font-size', {
        detail: x
      }));
    }, 32, 'change');
    ...
    bindCheckboxToFunction(lyricZoom, x => {
      document.dispatchEvent(new CustomEvent('rnp-lyric-zoom', {
        detail: x
      }));
    }, false);
    bindCheckboxToFunction(lyricFade, x => {
      document.dispatchEvent(new CustomEvent('rnp-lyric-fade', {
        detail: x
      }));
    }, false);
    bindCheckboxToFunction(lyricBlur, x => {
      document.dispatchEvent(new CustomEvent('rnp-lyric-blur', {
        detail: x
      }));
    }, false);
    bindCheckboxToClass(lyricRotate, 'lyric-rotate', false, x => {
      document.dispatchEvent(new CustomEvent('rnp-lyric-rotate', {
        detail: x
      }));
    });
    bindSliderToFunction(RotateCurvature, x => {
      document.dispatchEvent(new CustomEvent('rnp-rotate-curvature', {
        detail: x
      }));
    }, 25, 'change');
    bindSelectGroupToClasses(karaokeAnimation, 'float', x => `rnp-karaoke-animation-${x}`, x => {
      document.dispatchEvent(new CustomEvent('rnp-karaoke-animation', {
        detail: x
      }));
    });
    bindSelectGroupToClasses(currentLyricAlignmentPercentage, '50', x => `rnp-current-lyric-alignment-${x}`, x => {
      document.dispatchEvent(new CustomEvent('rnp-current-lyric-alignment-percentage', {
        detail: parseInt(x)
      }));
    });
    bindCheckboxToFunction(lyricStagger, x => {
      document.dispatchEvent(new CustomEvent('rnp-lyric-stagger', {
        detail: x
      }));
    }, true);
    bindSelectGroupToClasses(lyricAnimationTiming, 'smooth', x => `rnp-lyric-animation-timing-${x}`);
    bindCheckboxToFunction(lyricGlow, x => {
      document.dispatchEvent(new CustomEvent('rnp-lyric-glow', {
        detail: x
      }));
    }, true);
    ...
    bindSliderToFunction(lyricOffsetSlider, ms => {
      ms = parseInt(ms);
      document.dispatchEvent(new CustomEvent('rnp-global-offset', {
        detail: ms
      }));
      lyricOffsetNumber.innerHTML = `${['-', '', '+'][Math.sign(ms) + 1]}${(Math.abs(ms) / 1000).toFixed(1).replace(/\.0$/, '')}s`;
      if (ms === 0) lyricOffsetTip.innerHTML = '未设置';else lyricOffsetTip.innerHTML = ms > 0 ? '歌词提前' : '歌词滞后';
      if (ms === 0) lyricOffsetReset.classList.remove('active');else lyricOffsetReset.classList.add('active');
      shouldSettingMenuReload[isFM ? 1 : 0] = true;
  ...
  const initTabs = menu => {
    ...
    const setActive = name => {
      ...
      active = name;
      tabs.forEach(x => {
        if (x.dataset.tab === name) x.classList.add('active');else x.classList.remove('active');
      });
    };
    tabs.forEach(x => {
  ...
  if (!isFM) document.querySelector('.g-single').appendChild(settingsMenu);else document.querySelector('#page_pc_userfm_songplay').appendChild(settingsMenu);
  initSettings();
...
plugin.onLoad(async p => {
  ...
  new MutationObserver(async () => {
    ...
    if (document.querySelector('.g-single:not(.patched)')) {
      ...
      ReactDOM.render(<Background type={getSetting('background-type', 'fluid')} image={await waitForElementAsync('.n-single .cdimg img')} />, background);
      ...
      setTimeout(async () => {
        ReactDOM.render(<MiniSongInfo image={await waitForElementAsync('.n-single .cdimg img')} infContainer={await waitForElementAsync('.g-single .g-singlec-ct .n-single .mn .head .inf')} />, miniSongInfo);
  ...
  const patchFM = async () => {
    if (document.querySelector('#page_pc_userfm_songplay:not(.patched)')) {
      ...
      ReactDOM.render(<Background type={getSetting('background-type', 'fluid')} image={await waitForElementAsync('#page_pc_userfm_songplay .fmplay .covers')} isFM={true} imageChangedCallback={useCallback(dom => {
        if (!dom) return;
