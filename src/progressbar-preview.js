...
function useRefState(initialValue) {
  ...
  return [valueRef, value.current, updateValue];
}
...
export function ProgressbarPreview(props) {
  ...
  const updateHoverPercent = () => {
    ...
    if (_lyrics.current) {
      ...
      if (cur == _lyrics.current.length - 1 && _lyrics.current[cur].duration && currentTime > _lyrics.current[cur].time + _lyrics.current[cur].duration + 500) {
  ...
  const isPureMusic = lyrics && (lyrics.length === 1 || lyrics.length <= 10 && lyrics.some(x => (x.originalLyric ?? '').includes('纯音乐')) || document.querySelector('#main-player').getAttribute('data-log')?.includes('"s_ctype":"voice"') || lyrics[0]?.unsynced);
  return <div ref={containerRef} className={`progressbar-preview ${visible && !isPureMusic ? '' : 'invisible'}`}>
			{lyrics && lyrics[currentLine]?.originalLyric && <div className="progressbar-preview-number">{currentNonInterludeIndex} / {nonInterludeCount}</div>}
			{lyrics && lyrics[currentLine]?.dynamicLyric && <div className="progressbar-preview-line-karaoke">
						{lyrics[currentLine].dynamicLyric.map((word, i) => {
        const percent = (currentTime - word.time) / word.duration;
        return <span key={i} className={`progressbar-preview-line-karaoke-word ${percent >= 0 && percent <= 1 ? 'current' : ''} ${percent < 0 ? 'upcoming' : ''}`} style={{
          '-webkit-mask-position': `${100 * (1 - Math.max(0, Math.min(1, (currentTime - word.time) / word.duration)))}%`
			...
			{lyrics && !lyrics[currentLine]?.dynamicLyric && lyrics[currentLine]?.originalLyric && <div className="progressbar-preview-line-original">{lyrics[currentLine]?.originalLyric}</div>}
			{lyrics && lyrics[currentLine]?.originalLyric == '' && <div className="progressbar-preview-line-original">♪</div>}
			{lyrics && lyrics[currentLine]?.translatedLyric && <div className="progressbar-preview-line-translated">{lyrics[currentLine]?.translatedLyric}</div>}
			{lyrics && lyrics[currentLine] && <div className="progressbar-preview-subprogressbar">
						<div className="progressbar-preview-subprogressbar-inner" ref={subprogressbarInnerRef}></div>
					</div>}
			{lyrics && lyrics[currentLine] && <div className="progressbar-preview-line-time">
